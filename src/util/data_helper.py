...
class DataHelper:
    ...
    def format_td_amount(self, row):
        ...
        amount = float(f"-{amount.splitlines()[0]}") if amount[0] != "-" else float(amount[1:].splitlines()[0])
        return amount
    ...
    def format_table(self, data):
        ...
        for row in data:
            month = row[0][:3].upper()
            day = f"0{row[0][3:].strip()}" if len(row[0][3:].strip()) == 1 else f"{row[0][3:].strip().splitlines()[0]}"
